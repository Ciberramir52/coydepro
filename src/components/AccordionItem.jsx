    ...
    return (
        <div className="accordion-item">
            <h2 className="accordion-header">
                <button className="accordion-button bg-primary fs-2 d-flex gap-4 f-title text-light" type="button" data-bs-toggle="collapse" data-bs-target={`#${icon}`} aria-expanded="true" aria-controls={icon}>
                    <i className={`bi ${icon} text-secondary fs-1`} />
                    {title}
                </button>
            </h2>
            <div id={icon} className="accordion-collapse collapse show" data-bs-parent="#accordionExample">
                <div style={{ background: "#fff" }} className="accordion-body border border-dark">
                    ...
                    <ol>
                        {
                            explanation.map(step => <li className="fs-4">{step}</li>)
                        }
                    </ol>
